...
const DIGITAL_SIGNATURE_REQUIRED_DATE = new Date('2025-09-29T07:00:00.000Z');

// format MMDDCCYY as start of date in CA
function _formatMonthDayYear(dateStr) {
  const month = dateStr.slice(0, 2);
  const day = dateStr.slice(2, 4);
  const year = dateStr.slice(4);
  return `${year}-${month}-${day}T07:00:00.000Z`;
}
 ...
 *   'vcbRequired': `true` if AAMVA issuer data is valid and VCB data is
 *     required due to data or option.
 *   'vcbPresent': `true` if AAMVA issuer data is valid and VCB data is present.
 *   'error': Error details if not verified.
 *   'debug': Debug details when requested.
 */
export async function verify({
  ...
  const result = {
    // issuer identifier and DAJ fields match
    issuerValid: false,
    // VCB required due to data date or option
    vcbRequired: requireVcb,
    // VCB data found
    vcbPresent: false,
    // other checks pass and VCB data verified
    verified: false
  };
  if(debug) {
    result.debug = {};
  ...
  // check issuer
  // issuerValid set after DAJ check later
  ...
  // update result field
  result.vcbRequired = vcbRequired;
  ...
  try {
    ...
    const cborldBytes = fromBase64({base64String: encodedVcb});
    if(debug) {
